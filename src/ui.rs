...
pub(crate) fn run_ui(metrics_receiver: Receiver<MetricsSummary>) -> Result<(), Box<dyn Error>> {
    // setup terminal
    enable_raw_mode()?;
    ...
    let res = run_app(&mut terminal, metrics_receiver);
...
fn run_app<B: Backend>(
    ...
    metricyas_receiver: Receiver<MetricsSummary>,
) -> io::Result<()> {
    while let Ok(metrics_summary) = metricyas_receiver.recv() {
...
fn ui<B: Backend>(f: &mut Frame<B>, metrics: &MetricsSummary) {
    ...
    // let ok_durations: Vec<u64> = metrics.ok_durations.iter().map(|d| d.as_millis() as u64).collect();
    ...
    // let error_durations: Vec<u64> = metrics.error_durations.iter().map(|d| d.as_millis() as u64).collect();
